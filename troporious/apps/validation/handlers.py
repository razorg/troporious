...
from apps.validation.models import LiveSession
...
class PlaygroundLiveHandler(webapp.RequestHandler):
    def get(self):
        init_number = self.request.get('init_number')
        if not init_number:
            self.response.set_status(400)
            return self.response.out.write('No init_number parameter provided')
        ...
        tropo.tropo_run_script({'session_id':new_session.key().id(),'init_number':init_number})
    ...
    def post(self):
        ...
        if not session:
            ...
            return self.response.out.write('no such session')
        ...
        if _from == 'tropo':
            ...
            if action == 'get_next':
                ...
                session.put()
                client_message = {'type':'msg','what':'dequeue', 'msg':'action "%s" dequeued' % new_action}
